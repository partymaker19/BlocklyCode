...
export const tasks: Record<TaskId, TaskDef> = {
  ...
  first_function: {
    ...
    title: (lang) =>
      lang === "ru"
        ? "Задача 24: Моя первая функция"
        : "Task 24: My first function",
    description: (lang) =>
      lang === "ru"
  ...
  function_with_param: {
    ...
    title: (lang) =>
      lang === "ru"
        ? "Задача 25: Функция с параметром"
        : "Task 25: Function with a parameter",
    description: (lang) =>
      lang === "ru"
  ...
  function_return: {
    ...
    title: (lang) =>
      lang === "ru"
        ? "Задача 26: Функция с возвратом"
        : "Task 26: Function with a return value",
    description: (lang) =>
      lang === "ru"
...
/** Есть ли у блока предок заданного типа (через цепочку родителей). */
function hasAncestorOfType(block: any, type: string): boolean {
...
async function validateFirstFunction(
  ...
  const greetLines = lines.filter((l) =>
    /^(привет, мир!|hello, world!)$/i.test(l),
  );
...
async function validateFunctionReturn(
  ...
  let stars = 0;
  if (ok) {
    if (usedDefReturn && returnAttached && callReturnCount >= 1 && count <= 11)
      stars = 3;
